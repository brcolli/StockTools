...
class SpamModelLearning(ModelLearning):
    ...
    def predict_and_score(self, csv, affect_parameter_scores=False):
        ...
        :return: Generated scores including Accuracy, Precision, Recall, and F-Score as well as Numerical data with
                total tweets predicted on, true positives, false positives, true negatives, false negatives
        :rtype: dict(
                    'Accuracy': float, 'Precision': float, 'Recall': float, 'F-Score': float, 'Numerical':
                        dict('Total': int, 'True Positives': int, 'False Positives': int, 'True Negatives': int,
                        'False Negatives': int)
                    )
        """
        y = pd.read_csv(csv)['Label'].tolist()
        y1 = self.predict(csv)
        scores = Utils.calculate_ml_measures(y1, y)  # Format: accuracy, precision, recall, f1, (total, tp, fp, tn, fn)
        if affect_parameter_scores:
            self.parameters.accuracy, self.parameters.precision, self.parameters.recall, self.parameters.f_score = \
                scores[0:4]

        keys = ['Accuracy', 'Precision', 'Recall', 'F-Score', 'Numerical']
