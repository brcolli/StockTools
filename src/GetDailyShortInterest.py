...
def load_short_interest_text_and_write_to_csv(filename):
    ...
    write_data_to_file_no_newline(output, data)
...
def get_regsho_daily_short_to_csv(ymd):
    ...
    tcm = TCM()
    qs_df = generate_quotes_df(tcm, tickers_chunks)
    fs_df = generate_fundamentals_df(tcm, tickers_chunks)

    # Set new index
    df = df.set_index('Symbol')

    (prev_short_perc, prev_vol_perc) = get_past_short_vol(df, tickers, tcm, ymd, short_file_prefix)

    df = update_short_df_with_data(df, qs_df, fs_df, prev_short_perc, prev_vol_perc)
...
def get_latest_short_interest_data():
    ...
    return get_regsho_daily_short_to_csv(ltd)
