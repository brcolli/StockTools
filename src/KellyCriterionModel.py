...
class KellyModel:
    def __init__(self, beg_capital=10000, edge=0.5, glr=1.15, fractional_kelly=0.25, l_ceiling=0.2, w_ceiling=0.5,
                 num_runs=100, tax_rate=0.1, snp=0.1478, expected_return=0.0008):
        self.beg_capital, self.edge, self.glr, self.fractional_kelly, self.l_ceiling, self.w_ceiling, self.num_runs =\
            beg_capital, edge, glr, fractional_kelly, l_ceiling, w_ceiling, num_runs
        ...
        self.keys = ['Ending Capital', 'Wins vs Losses', 'Largest Winning Streak', 'Largest Losing Streak',
    ...
    def single_run(self):
        bet = (self.edge - ((1-self.edge) / self.glr)) * self.fractional_kelly
    ...
    def to_dict(self):
        values = [self.ending_capital, self.win_v_loss, self.largest_win_streak, self.largest_loss_streak,
    ...
    def multi_runs(self, num):
        df = pd.DataFrame()

        for i in range(num):
            self.__init__()
            df = df.append(self.wrapper(), ignore_index=True)
            df = df[self.keys]
